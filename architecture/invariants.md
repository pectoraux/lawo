...
**Test.** Architecture test `provenance-on-decisions` rejects any persisted decision snapshot whose `provenance` array is empty or whose entries omit required fields. As of ADR 0013, `provenance` (along with `truthLevel`, `state`, `computedAt`) is always server-generated by the `DecisionEngine` — the client cannot supply these fields. `POST /api/decisions` is removed; the only persist path is `POST /api/state` with `persist: true`, which runs the engine server-side.

**Machine-checkable:** YES — `provenance-on-decisions` (static) + `runtime-decision-integrity` (see tests/runtime-security/run.ts, INTEGRITY-001 through INTEGRITY-003).
...
**Test.** Architecture test `tenant-data-isolation` issues queries from a tenant context and asserts no other tenant's facts are returned; integration test confirms global retrieval does not include tenant-private facts. As of ADR 0012, tenant isolation is enforced at the data-access boundary by `effectiveTenantScope` / `canAccessTenant` (`src/lib/auth/guards.ts`) — every authenticated API route derives the tenant scope from the authenticated session, not from client-supplied `tenantId` / `subjectId` query params.

**Machine-checkable:** YES — `runtime-tenant-isolation` (see tests/runtime-security/run.ts, AUTHZ-001 through AUTHZ-006) + static boundary check `privileged-routes-check-authz`.
