...
---
Task ID: ARCH-DOCS-2
Agent: Architecture Docs (Authorization Sprint)
Task: ADRs 0012-0016 + invariants.md enforcement update + architecture test categories doc

Work Log:
- Read all required context: worklog.md (Tasks 1–9 + ARCH-DOCS + ARCH-TESTS + HARDENING-SPRINT-REPORT), architecture/decisions/ (existing 11 ADRs 0001–0011 for §36 template format), architecture/invariants.md (I1–I18 with Machine-checkable markers), architecture/architecture-tests/README.md + run.ts (the existing 14-check static suite, structure and check naming), src/lib/auth/guards.ts (the new requireUserWithScope / requireAdminWithScope + effectiveTenantScope / canAccessTenant helpers), src/app/api/decisions/route.ts (GET only; POST removed; subjectId AND-scoped with tenant filter), src/app/api/state/route.ts (DecisionRequest in → DecisionRecord out; tenantId from user.tenantId), src/platform/audit/AuditLog.ts (record() durable-throws vs recordBestEffort() synthesized non-durable; AuditPersistenceError exported; forSubjectInTenant added), src/lib/rate-limit-store.ts (RateLimitStore interface, InMemoryRateLimitStore, SharedRateLimitStore stub, getRateLimitStore/setRateLimitStore factory, RATE_LIMIT_ALLOW_IN_MEMORY env var).
- Created 5 new ADRs under /home/z/my-project/architecture/decisions/, each following the §36 template (Status: ACCEPTED, Date, Decider, Supersedes, Superseded by, Context, Decision, Alternatives considered, Consequences, Invariants affected, Migration implications, References) — matching the exact format of ADRs 0006–0011. Cross-references I-numbers (I5, I6, I9, I12, I13, I15, I18) and existing ADRs (0006–0011) and the new companion ADRs.
  - 0012-tenant-authorization-boundary.md — records the decision that every authenticated API route derives the effective tenant scope from the authenticated session (not from client-supplied tenantId/subjectId). Documents requireUserWithScope / requireAdminWithScope + effectiveTenantScope / canAccessTenant in src/lib/auth/guards.ts. Documents the route-handler pattern (tenantFilter from session; subjectId AND-scoped). Alternatives rejected: Postgres RLS (too rigid for admin platform-wide reads), middleware that injects tenantId (doesn't cover subjectId bypass), OPA/Cedar policy layer (premature), per-route hand-written checks (what produced the gap). Consequences: non-admin users see only their own tenant's data; admin platform-wide reads are explicit and auditable via ?platformWide=true; writes always own-tenant (no admin platform-wide write). Invariants: I9 (now enforced at data-access boundary, was behavioral-only), I12 (capability boundaries strengthened), I18 (auth sprint, not architecture change).
  - 0013-decision-integrity-separation.md — records the separation of DecisionRequest (client input) from DecisionRecord (server-authored authoritative output). POST /api/decisions is REMOVED; POST /api/state with persist:true is the only persist path. Documents the StateRequestBody type and explicitly lists ABSENT fields (decisionId, truthLevel, provenance, state, computedAt, tenantId — server-generated or session-derived). Alternatives rejected: HMAC-sign the client state (trusts client computation), accept-but-re-verify (redundant), keep POST /api/decisions but ignore authoritative fields (confusing API surface), server-side computation token (no benefit), move persist into the engine (kernel-imports violation). Consequences: client cannot forge truthLevel/provenance; persisted DecisionRecord.tenantId is user.tenantId (ADR 0012); GET /api/decisions unchanged. Invariants: I5 (extends to "client non-authoritative"), I6 (provenance always server-generated), I9, I13 (historical reproducibility strengthened), I18.
  - 0014-audit-durability-policy.md — records the split of audit recording into durable record() (throws AuditPersistenceError on DB failure; security-sensitive operations roll back the action) and best-effort recordBestEffort() (synthesized event with payload._durable:false on failure; informational events only). Documents the AuditLog interface, the AuditPersistenceError class, the security-sensitive vs informational event lists (waitlist.approve/reject, auth.set_password, auth.signin_success/failure, auth.role_change = durable; decision.compute, decision.persist = best-effort). Alternatives rejected: always durable (too strict for informational events), always best-effort (loses audit integrity — the original problem), single method with durable:boolean parameter (runtime choice is fragile), DB transaction as durability boundary (correct where possible but not universal), async audit queue (best-effort with extra steps), external SIEM (consumer not replacement). Consequences: privileged operations fail loudly if audit can't persist (503 + rollback); informational events never break the request; audit trail has no silent holes for privileged operations; _durable:false flag visible in payload. Invariants: I6 (audit trail durable for privileged operations), I12 (capability contingent on recording), I18.
  - 0015-identity-source-of-truth.md — records the deletion of src/platform/identity/Identity.ts (the legacy demoIdentities/getIdentity stub). The authoritative identity source is the NextAuth session backed by the User table. Documents the single-source-of-truth flow (scripts/seed-users.ts + waitlist flow → User table → NextAuth Credentials → JWT session → SessionUser → AuthGate UI). Documents that DEMO_ACCOUNTS in src/lib/auth/demoAccounts.ts is a UI-only list of emails (no role/tenantId assertions — those come from the session at sign-in). Alternatives rejected: keep Identity.ts as a "fallback" (split-brain), merge demoIdentities into DB as a thin read (already done — that's DEMO_ACCOUNTS), config file (drift risk), delete AuthGate buttons (useful for demo), use NextAuth providers array (that's auth mechanism not identity source). Consequences: exactly one identity source (User table); /api/orient no longer returns identities field; AuthGate reads DEMO_ACCOUNTS. Invariants: I9 (tenantId authoritative only from User table), I15 (cleanup not architecture change — recorded for history), I18.
  - 0016-rate-limit-store-abstraction.md — records the abstraction of rate limiting behind a RateLimitStore interface with InMemoryRateLimitStore (per-instance, NOT production-grade) and SharedRateLimitStore (stub that throws "not implemented"). Documents getRateLimitStore() factory (warns in production unless RATE_LIMIT_ALLOW_IN_MEMORY=1) and setRateLimitStore() for future injection. The interface is intentionally synchronous (microseconds-fast for in-memory; future Redis impl may require an async interface change — deferred to a future ADR). Alternatives rejected: add Upstash Redis now (premature for the platform's scale), remove rate limiting (insecure), Vercel Edge Middleware (per ADR 0011 — runtime mismatch), third-party service (deployment-architecture concern not application-architecture), async interface (microtask hop for no current benefit), DI container (overkill). Consequences: call sites don't change when a shared store is added; limitation explicitly documented and acknowledged via console warning + env var; SharedRateLimitStore stub fails loudly on misconfiguration. Invariants: none directly (security hardening per I18); ADR 0011's per-instance limitation is acknowledged not removed.
- Updated /home/z/my-project/architecture/invariants.md with the new runtime-test enforcement:
  - I9 (tenant data isolation): added sentence pointing to ADR 0012 and the effectiveTenantScope / canAccessTenant helpers; changed Machine-checkable line from "NO — tenant isolation is behavioral; runtime verification with real data is required..." to "YES — runtime-tenant-isolation (see tests/runtime-security/run.ts, AUTHZ-001 through AUTHZ-006) + static boundary check privileged-routes-check-authz."
  - I6 (provenance): added sentence pointing to ADR 0013 (POST /api/decisions removed; POST /api/state with persist:true is the only persist path; authoritative fields always engine-generated); changed Machine-checkable line from "YES — provenance-on-decisions (see architecture-tests/run.ts)" to "YES — provenance-on-decisions (static) + runtime-decision-integrity (see tests/runtime-security/run.ts, INTEGRITY-001 through INTEGRITY-003)."
- Created /home/z/my-project/architecture/architecture-tests/CATEGORIES.md per the exact spec — distinguishes Static Boundary Tests (14 checks, <100ms, source structure, run.ts, bun run arch-test) from Runtime Invariant Tests (15 checks, ~10-30s, behavioral, tests/runtime-security/run.ts, bun run runtime-test, requires dev server). Lists all 14 static checks (I1/I2/I3 kernel-imports-no-verticals, I5 kernel-imports-no-llm, I6 provenance-on-decisions, I7 temporal-metadata-on-rules, I10 package-dependency-rules, I11 packages-do-not-mutate-kernel, I16 no-feature-specific-hacks-in-kernel, AUTHZ privileged-routes-check-authz, SEC no-secrets-in-client-code, SEC audit-payload-sanitizer, SEC no-remote-seeding, SEC csrf-on-mutations) and all 15 runtime checks (AUTHZ-001 through AUTHZ-008, INTEGRITY-001 through INTEGRITY-003, WAITLIST-001/002, SETPW-001/002). Documents the "Why both categories are required" rationale (the authorization sprint's gap passed static, failed runtime — a route can have a guard call in source but still accept client-supplied tenantId). Documents CI integration (both suites required on PRs touching src/kernel/, src/intelligence/, src/procedures/, src/situations/ (static); src/app/api/, src/lib/auth/, src/platform/ (static + runtime); prisma/schema.prisma (runtime)).
- Updated /home/z/my-project/architecture/README.md:
  - Added 5 rows to the Decisions table (0001–0011 → 0001–0016, total 16 ADRs) with one-line descriptions of each new ADR.
  - Added a row to the architecture-tests table pointing to architecture-tests/CATEGORIES.md.
  - Added a new "Test Categories" section between "The 18 Architecture Invariants" and "How to Use This Directory" — summarizes the two categories (Static Boundary Tests + Runtime Invariant Tests), the run commands, the check counts, the "why both required" rationale (pointing at ADRs 0012 and 0013 as the case study), and the CI integration rule.
- Did NOT modify any code files. Did NOT touch existing ADRs (0001–0011), existing contracts, constitution.md, or the existing architecture-tests/run.ts script. All changes are markdown-only.

Stage Summary:
- ADRs added (5): decisions/0012-tenant-authorization-boundary.md, decisions/0013-decision-integrity-separation.md, decisions/0014-audit-durability-policy.md, decisions/0015-identity-source-of-truth.md, decisions/0016-rate-limit-store-abstraction.md. Total ADRs: 16.
- Invariants updated (2): I9 (Machine-checkable: YES — runtime-tenant-isolation AUTHZ-001..006 + static privileged-routes-check-authz) and I6 (Machine-checkable: YES — provenance-on-decisions static + runtime-decision-integrity INTEGRITY-001..003). Both updated to reflect ADRs 0012 and 0013 enforcement at the data-access / decision-integrity boundary.
